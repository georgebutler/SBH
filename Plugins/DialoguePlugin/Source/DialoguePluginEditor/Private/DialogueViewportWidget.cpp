...
{		
	...
	CommentCalloutArrow = FAppStyle::GetBrush(TEXT("Graph.Node.CommentArrow"));
	CommentCalloutBubble = FAppStyle::GetBrush(TEXT("Graph.Node.CommentBubble"));
	CommentFont = FAppStyle::GetFontStyle(TEXT("Graph.Node.CommentFont"));
...
void SDialogueViewportWidget::SpawnNodes(int32 IdToFocus)
{
	...
	for (auto &Node : Dialogue->Data)
	{
		...
		.AutoSize(true)
		[
			SAssignNew(tempWidgetReference, SDialogueNodeWidget) // tempWidgetReference is TSharedPtr<SDialogueNodeWidget>
			.Id(Node.id)
			.NodeIndex(i)
			.Dialogue(Dialogue)
			.Owner(this)
...
int32 SDialogueViewportWidget::OnPaint(const FPaintArgs& Args, const FGeometry& AllottedGeometry, const FSlateRect& MyClippingRect, FSlateWindowElementList& OutDrawElements, int32 LayerId, const FWidgetStyle& InWidgetStyle, bool bParentEnabled) const
{
	...
	if (bShowSoftwareCursor)
	{
		...
		FSlateDrawElement::MakeBox(
			...
			AllottedGeometry.ToPaintGeometry(Brush->ImageSize, FSlateLayoutTransform(1.0f, TransformPoint(1.0f, SoftwareCursorPosition - (Brush->ImageSize / 2)))),
			Brush
			);
	}
...
{
	CommentFont.Size = FAppStyle::GetFontStyle(TEXT("Graph.Node.CommentFont")).Size + GetZoomLevel() - 12;
